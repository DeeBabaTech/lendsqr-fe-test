...
function Header() {
  ...
  return (
    <div className='flex justify-between items-center p-5 bg-white'>
      {menuIcon ? <img src={exit} alt='' className='md:hidden block w-7 bg-white' onClick={handleIcon} /> : 
                  <img src={menu} alt='' className='md:hidden block w-7 bg-white' onClick={handleIcon} /> 
                }
      <img src={logo} alt='' className='md:w-fit w-40'/>
      <div className='md:flex items-center hidden'>
        <input type="text" placeholder="Search for anything" className='rounded-l-lg px-5 w-80 py-2 outline outline-slate-300'/>
        <div className='bg-[#39cdcc] p-[0.9rem] rounded-r-lg'>
          <img src={search} alt='' />
        </div>
      </div>
      <div className='flex items-center'>
        <p className='mr-7 underline hidden md:block'>Docs</p>
        <img src={bell} alt=''  className='mr-5 hidden md:block'/> 
        <img src={avatar} alt='' className='rounded-full mr-3'/> 
        <p className='mr-2 hidden md:block'>Adedeji</p>
        <img src={dropdown} alt='' className='rounded-full hidden md:block'/> 
      </div>

      {/* menu options for mobile view only */}

      <div className={menuIcon ? 'mt-20 absolute left-0 top-0 bg-white z-20 w-9/12 transition-all duration-700' : 'absolute -left-full' }>
        <ul>
